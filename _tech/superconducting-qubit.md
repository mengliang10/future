---
layout: tech_node
title: "Superconducting Qubit"
subtitle: "Josephson junction qubit at millikelvin; IBM Eagle/Heron, Google Willow"
category: Quantum
stage: early_commercial
confidence_label: Speculative
confidence_score: 0.168
est_year: 
est_year_range: ""
horizon: "Deployed"
timeline_confidence: 99
monetizable_year: "Now–2027"
monetizable_confidence: 88
source_count: 25
stocks: [RGTI, IBM, INTC]
lead_stock: RGTI
lead_stock_confidence: 90
permalink: /tech/superconducting-qubit/
---

## What This Is

Josephson junction qubit at millikelvin; IBM Eagle/Heron, Google Willow. **Achieved.**

Early commercial products exist. Market is forming with limited but growing deployment. Our research database tracks **25 independent sources** on this technology.

## Status

| Attribute | Value |
|-----------|-------|
| Development Stage | **Early Commercial** |
| Confidence Level | <span class="conf-badge conf-speculative">Speculative</span> |
| Confidence Score | **+0.168** |
| Estimated Reality | **Ongoing / Deployed** |
| Timeline Confidence | **99%** |
| Monetizable By | **Now–2027** *(est. confidence: 88%)* |
| Horizon | Deployed |
| Independent Sources Tracked | 25 |

## Market Race

**3-Way Race** — 3 companies with meaningful exposure

| Ticker | Company | Confidence | Position | Thesis |
|--------|---------|-----------|----------|--------|
| [RGTI](/future/stocks/rgti/) | Rigetti Computing | 90% | DIRECT | Superconducting qubit systems on Rigetti cloud **← Leader** |
| [IBM](/future/stocks/ibm/) | IBM Corporation | 80% | DIRECT | 1000+ qubit Heron systems; IBM Quantum Network |
| [INTC](/future/stocks/intc/) | Intel Corporation | 50% | secondary | Intel Tunnel Falls qubit chip; Oregon research fab; Horse Ridge cryogenic c |

**Current leader:** [RGTI](/future/stocks/rgti/) with **90%** exposure confidence based on our research.

## Timeline Intelligence

| Milestone | Estimate | Confidence |
|-----------|----------|-----------|
| Technology Reality | **Ongoing / Deployed** | 99% |
| Commercially Monetizable | **Now–2027** | 88% |

High timing confidence (99%) — multiple corroborating sources confirm this timeline.

## What This Enables

Advancement here directly benefits or unblocks:

- [Quantum Error Correction (Surface Code)](/future/tech/quantum-error-correction-surface-code/) — *enables*

## Investment Exposure

[RGTI](/future/stocks/rgti/)  [IBM](/future/stocks/ibm/)  [INTC](/future/stocks/intc/)

See each stock page for revenue drivers, milestones, and risk factors.
